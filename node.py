...
class Node:
	...
	def leave_handler(self, req_stamp):
		...
		if(not self.pending.empty()):
			...
			self.timestamp+=1
			self.send_msg("Ya_GOOD "+str(self.node_id) +' '+str(self.timestamp), self.sock[int(waiting_node[1])])
	...
	def yield_handler(self, yielder):
		if(not self.pending.empty()):
			...
			self.send_msg("Ya_GOOD_Yielded "+str(self.node_id) +' '+str(self.timestamp), self.sock[int(waiting_node[1])])
